...
class Pizza:
    def __init__(self, baseprice, toppingcost, toppings):
        self.baseprice = baseprice
        self.toppingcost = toppingcost
        self.toppings = toppings

    def CalculatePrice(self):
        return self.baseprice + (self.toppingcost * self.toppings)


class DiscountedPizza(Pizza):
    def CalculatePrice(self):
        regularprice = super().CalculatePrice()  
        if self.toppings > 3:
...
toppingcost = float(input("Enter Topping Cost = "))
toppings = int(input("Enter Number of Toppings = "))

regularprice = Pizza(baseprice, toppingcost, toppings)
discounted_pizza = DiscountedPizza(baseprice, toppingcost, toppings)

withoutdiscount = regularprice.CalculatePrice()
discounted = discounted_pizza.CalculatePrice()

print("Price Without Discount =", withoutdiscount)
print("Price With Discount =", discounted)
