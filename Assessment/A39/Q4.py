'''
4.
Assignment: Scholarship Eligibility System (Using filter(), map(), and sorted() with Lambda 
Expressions)
...
Note: Use filter() to select eligible students, map() to add the bonus marks, and sorted() to 
display the final marks in descending order. All three operations must use lambda expressions.
...
result = sorted(map(lambda x: x+5, filter(lambda x: x >= 75, marks)), reverse=True)
print(list(result))