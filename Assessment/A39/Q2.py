...
A software company wants to reward its high-performing employees with a 10% bonus. However, only 
employees earning ₹50,000 or more are eligible for
 the bonus.
...
for i in range(n):
    s = int(input(f"Enter Salary {i+1} = "))
    salaries.append(s)

result = map(lambda x: x+(x*10/100), filter(lambda x: x >= 50000,salaries))
print(list(result))